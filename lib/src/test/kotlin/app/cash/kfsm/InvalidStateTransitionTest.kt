...
class InvalidStateTransitionTest : StringSpec({
  "with single from-state has correct message" {
    InvalidStateForTransition(LetterTransition(States(A), B), Letter(E, id = "my_letter")).message shouldBe
  ...
  "exposes transition and state" {
    val error = InvalidStateForTransition(LetterTransition(States(A), B), Letter(E, "my_letter"))

    error.getState<String, Letter, Char>() shouldBe E
  }
})
